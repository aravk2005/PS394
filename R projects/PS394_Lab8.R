...
##simple linear regression
#center your variables (if you want/need to) - using z score method here
data$zSOC_mean <- ((data$SOC_mean - mean(data$SC_mean, na.rm=TRUE))/sd(data$SC_mean, na.rm=TRUE))  #convert you variable to z scores

#get scatterplots for your variables
plot(data$GPA ~ data$zSOC_mean, data = data) #get a scatterplot with self compassion on the x axis and GPA on the y axis

#run regression
model3 <- lm(GPA ~ zSC_mean, data = data)
summary(model3) #simple linear regression with self-compassion only

##multiple regression 

#center your additional variables (if you want/need to) - using z score method here
data$zAge <- ((data$Age - mean(data$Age, na.rm=TRUE))/sd(data$Age, na.rm=TRUE)) #convert you variable to z scores

#get scatterplot for your additional variables
plot(data$GPA ~ data$zAge, data = data) #get a scatterplot with self compassion on the x axis and GPA on the y axis

#run regression
model4 <- lm(GPA ~ zSOC_mean + zAge, data = data)
summary(model4) #multiple regression with self-compassion and exercise

#compare models: Rsquare change = 0.03472 (R2 from model1) - 0.02838 (R2 from model2) = 0.00634 (0.634%)
anova(model3, model4) #compare the models - p value for R square change
