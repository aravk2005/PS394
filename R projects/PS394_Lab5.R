...
data <- read.csv("PS394 Class Dataset F2025_MB.csv") #let's load it back in
...
data$Pets[data$ID == '128'] <- 0 #recode null amount of pets to 0 
...
#Linear correlation between the MathGrade variable and GPA variable
corr.test(data$MathGrade, data$GPA)

#Correlation matrix between MathGrade, GPA, and Exercise
rcorr(data$MathGrade, data$GPA, data$Exercise)
